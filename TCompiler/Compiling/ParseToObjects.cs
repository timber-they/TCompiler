...
namespace TCompiler.Compiling
{
    ...
    public static class ParseToObjects
    {
            ...
            foreach (var cLine in splitted)
            {
                ...
                switch (type)
                {
                    ...
                    case CommandType.InterruptServiceRoutine:
                    {
                        ...
                        var c = 0;
                        if (!external &&
                            (!int.TryParse (tLine.Split (new [] {' '}, StringSplitOptions.RemoveEmptyEntries) [1],
                                            out c) ||
                             c > 65536 ||
                             c <= 0))
                    ...
                    case CommandType.Collection:
                    {
                        ...
                        var c = new Collection (CurrentByteAddress,
                                                GetVariableDefinitionName (
                                                    string.Join ("", tLine.SkipWhile (c1 => c1 != '#')).
                                                           Trim ('#', ' ')), count);
        ...
        private static void AddMethods (IEnumerable<CodeLine> lines)
        {
            foreach (var cLine in lines)
            {
                ...
                if (GetCommandType (tLine) != CommandType.Method)
                    continue;
                var name = tLine.Split (new [] {' ', '['}, StringSplitOptions.RemoveEmptyEntries) [1].Split ('[').
                                 First ();
                if (!IsNameValid (name))
        ...
        private static bool IsNameValid (string name) => name.Length > 0 &&
                                                         GlobalProperties.InvalidNames.All (
                                                             s =>
                                                                 !s.Equals (name,
                                                                            StringComparison.
        ...
        private static Declaration GetDeclarationToVariable (Variable variable, string tLine)
        {
            if (_variableList.Any (
                var =>
                    var.Name.Equals (variable.Name, StringComparison.CurrentCultureIgnoreCase)))
        ...
        private static void CheckName (string name)
        {
            if (
                _variableList.Any (
                    variable =>
                        variable.Name.Equals (name, StringComparison.CurrentCultureIgnoreCase)))
        ...
        public static List<ReturningCommand> GetMethodParameterValues (string line, IReadOnlyList<Variable> parameters)
        {
            ...
            return
                rawValues.Select (
                    value =>
                        new TemporarOperationRepresentation (value).GeTemporaryReturning ().Item2?.
                                                                    GetReturningCommand ()).ToList ();
        }
        ...
        private static void ThrowExceptionIfTypeUnEqualAssignment (Tuple<Variable, ReturningCommand> pars)
        {
            ...
            if (t2 != null && t1 == t2)
                throw new InvalidVariableTypeException (GlobalProperties.CurrentLine,
                                                        pars.Item1?.Name ??
                                                        ((VariableCall) pars.Item2)?.Variable?.Name ??
                                                        pars.Item2?.ToString ());
        }
        ...
        public static Variable GetVariable (string variableIdentifier)
        {
            ...
            return variableIdentifier.Contains ('.')
                       ? new BitOfVariable (var.Address, index.ByteVariable, GlobalProperties.Label,
                                            GlobalProperties.Label,
